from PySide6.QtWidgets import (
    ...
    QFrame, QComboBox, QLineEdit, QCheckBox, QGroupBox,
...
class SettingsWidget(QWidget):
    def __init__(self):
        ...
        self.app_group.setStyleSheet(f"""
            QGroupBox {{
                font-weight: bold;
        ...
        self.dirs_group.setStyleSheet(f"""
            QGroupBox {{
                font-weight: bold;
                color: {style.purple_color};
                border: 1px solid {style.purple_color};
        ...
        self.upload_dir_button.clicked.connect(lambda: self.choose_directory(self.upload_dir_input))
        ...
        self.export_dir_button.clicked.connect(lambda: self.choose_directory(self.export_dir_input))
        ...
        self.video_group.setStyleSheet(f"""
            QGroupBox {{
                font-weight: bold;
                color: {style.purple_color};
                border: 1px solid {style.purple_color};
        ...
        self.upload_group.setStyleSheet(f"""
            QGroupBox {{
                font-weight: bold;
                color: {style.purple_color};
                border: 1px solid {style.purple_color};
    ...
    def load_settings(self):
        ...
        try:
            ...
            if 'notifications' in config:
                self.notifications_check.setChecked(config['notifications'].get('enabled', True))
                self.check_interval.setValue(config['notifications'].get('check_interval', 60))
            ...
            if 'video' in config:
                if 'default_quality' in config['video']:
                    self.quality_combo.setCurrentText(config['video']['default_quality'])
            ...
            if 'uploads' in config:
                ...
                if 'allowed_types' in config['uploads']:
                    ...
                    self.image_check.setChecked('image/jpeg' in types or 'image/png' in types)
    ...
    def save_settings(self):
        ...
        try:
            ...
            if self.zip_check.isChecked():
                allowed_types.extend(['application/zip', 'application/x-rar-compressed'])

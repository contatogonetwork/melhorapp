from PySide6.QtWidgets import (
    ...
    QScrollArea, QFrame, QSizePolicy, QComboBox, QGraphicsView,
    QGraphicsScene, QGraphicsRectItem, QGraphicsTextItem
)
from PySide6.QtCore import Qt, QSize, QRectF, QPoint
from PySide6.QtGui import QIcon, QColor, QPen, QBrush, QFont, QPainter
...
class TimelineWidget(QWidget):
    def __init__(self):
        ...
        self.member_filter.addItems(["Todos os Membros", "João Silva", "Maria Souza", "Carlos Lima", "Ana Costa"])
        ...
        self.activity_filter.addItems(["Todas as Atividades", "Captação", "Edição", "Entrega", "Aprovação"])
        ...
        self.status_filter.addItems(["Todos os Status", "Pendente", "Em andamento", "Concluído", "Atrasado"])
...
class TimelineView(QGraphicsView):
    ...
    def generate_example_timeline(self):
        ...
        team_members = ["João (Cinegrafia)", "Maria (Edição)", "Carlos (Drone)", "Ana (Coordenação)"]
    ...
    def add_tasks_for_member(self, member_index, y_pos, row_height, hour_width):
        ...
        if member_index == 0:  # João (Cinegrafia)
            ...
            self.add_task(1.5, 2.0, y_pos, row_height, hour_width, 
                         "Captação - Palco Principal", task_colors["captação"])
            
            # Tarefa 2: Captação de patrocinador
            self.add_task(4.0, 1.0, y_pos, row_height, hour_width, 
                         "Patrocinador A - Stand", task_colors["captação"])
            
            # Tarefa 3: Captação concluída
            self.add_task(7.5, 1.5, y_pos, row_height, hour_width, 
                         "Captação - Backstage", task_colors["concluído"])
        
        elif member_index == 1:  # Maria (Edição)
            # Tarefa 1: Edição em andamento
            self.add_task(2.0, 1.5, y_pos, row_height, hour_width, 
                         "Edição - Abertura", task_colors["edição"])
            
            # Tarefa 2: Entrega concluída
            self.add_task(5.0, 0.5, y_pos, row_height, hour_width, 
                         "Entrega - Reels Patrocinador", task_colors["concluído"])
            
            # Tarefa 3: Edição futura
            self.add_task(8.0, 2.0, y_pos, row_height, hour_width, 
                         "Edição - Teaser Final", task_colors["edição"])
        
        elif member_index == 2:  # Carlos (Drone)
            # Tarefa 1: Captação drone
            self.add_task(3.0, 1.0, y_pos, row_height, hour_width, 
                         "Captação Drone - Área Externa", task_colors["captação"])
            
            # Tarefa 2: Captação atrasada
            self.add_task(6.0, 0.5, y_pos, row_height, hour_width, 
                         "Captação Drone - Vista Geral", task_colors["atrasado"])
        
        elif member_index == 3:  # Ana (Coordenação)
            # Tarefa 1: Aprovação
            self.add_task(2.5, 0.5, y_pos, row_height, hour_width, 
                         "Aprovação - Material Inicial", task_colors["aprovação"])
            
            # Tarefa 2: Entrega
            self.add_task(4.5, 0.5, y_pos, row_height, hour_width, 
                         "Entrega - Stories", task_colors["entrega"])
            
            # Tarefa 3: Aprovação futura
            self.add_task(9.0, 1.0, y_pos, row_height, hour_width, 
                         "Aprovação - Teaser", task_colors["aprovação"])
    
    def add_task(self, start_hour, duration, y_pos, row_height, hour_width, title, color):
        # Calcular posições

...
class EventButton(QObject):
    ...
    def add_new_event(self, event_widget):
        ...
        if not event_name or event_client == "Selecione um cliente":
            self.show_message(
                "Erro", "Nome do evento e cliente são obrigatórios!", "error"
            )
            return
    ...
    def edit_event(self, event_name):
        ...
        new_name, ok = QInputDialog.getText(
            None, "Editar evento", "Novo nome do evento:", text=event_name
        )
        if ok and new_name:
    ...
    def add_team_member(self, widget):
        ...
        email_icon.clicked.connect(
            lambda: QDesktopServices.openUrl(QUrl(f"mailto:{email}"))
        )
        ...
        self.show_message(
            "Sucesso", f"Membro da equipe '{name}' adicionado com sucesso!"
        )
    ...
    def add_client(self, widget):
        ...
        responsible, ok2 = QInputDialog.getText(
            None, "Novo cliente", "Nome do responsável:"
        )
    ...
    def edit_team_member(self, name):
        ...
        new_name, ok = QInputDialog.getText(
            None, "Editar membro", "Novo nome:", text=name
        )
        if ok and new_name:
    ...
    def edit_client(self, name):
        """Edita um cliente"""
        new_name, ok = QInputDialog.getText(
            None, "Editar cliente", "Novo nome da empresa:", text=name
        )
        if ok and new_name:
    ...
    def save_briefing(self, widget):
        ...
        event_name = (
            widget.event_combo.currentText()
            if hasattr(widget, "event_combo")
            else "Evento Exemplo"
        )
        project_name = (
            widget.project_name_input.text()
            if hasattr(widget, "project_name_input")
            else "Projeto Exemplo"
        )
        content = (
            widget.content_text.toPlainText()
            if hasattr(widget, "content_text")
            else "Conteúdo do briefing"
        )

        if not content:
            self.show_message(
                "Erro", "O conteúdo do briefing não pode estar vazio!", "error"
            )
            return
        ...
        try:
            ...
            self.show_message(
                "Briefing Salvo",
                f"Briefing salvo com sucesso em:\n{file_path}",
            )
        except Exception as e:

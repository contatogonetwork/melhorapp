...
def create_event_team_table():
    ...
    try:
        ...
        check_sql = (
            "SELECT name FROM sqlite_master WHERE type='table' AND name='event_team'"
        )
        result = db.fetch_one(check_sql)
        if result:
            print(
                "Confirmado: a tabela event_team foi criada e existe no banco de dados."
            )
        else:
...
def associate_team_to_events():
    ...
    print(
        f"Encontrados {len(event_ids)} eventos e {len(member_ids)} membros da equipe."
    )

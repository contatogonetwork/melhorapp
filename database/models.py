...
class Briefing(BaseModel):
    ...
    def load_by_event(self, event_id):
        briefing = self.db.fetch_one("SELECT * FROM briefings WHERE event_id = ?", (event_id,))
        if briefing:
            self.id = briefing['id']
            self.event_id = briefing['event_id']
            self.general_info = briefing['general_info']
    ...
    def save(self):
        ...
        if self.id:
            ...
            return self.db.update("briefings", {
            ...
            # Criar novo briefing
            briefing_id = self.db.insert("briefings", {
...
class Sponsor(BaseModel):
    ...
    def save(self):
        if self.id:
            ...
            return self.db.update("sponsors", {
                'name': self.name
            }, {'id': self.id})
        else:
            # Criar novo patrocinador
            sponsor_id = self.db.insert("sponsors", {
                'event_id': self.event_id,
                'name': self.name
            })
    ...
    def add_action(self, action_data):
        if self.id:
            action_id = self.db.insert("sponsor_actions", {
                'sponsor_id': self.id,
...
class Video(BaseModel):
    ...
    def add_comment(self, user_id, comment, timestamp=None):
        if self.id:
            return self.db.insert("video_comments", {
                'video_id': self.id,
                'user_id': user_id,
                'comment': comment,
                'timestamp': timestamp,

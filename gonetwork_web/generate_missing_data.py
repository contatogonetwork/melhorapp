"""
Script para gerar dados de exemplo para as tabelas vazias.
"""
...
def init_missing_data():
    """
    Inicializa dados de exemplo no banco de dados especificamente para tabelas vazias.
    """
    ...
    try:
        ...
        # Verificar eventos e membros da equipe (necessários para criar outros dados)
        ...
        if event_count == 0:
            print("[ℹ️] Criando eventos de exemplo...")
            create_sample_events(cursor)

        # Se não houver membros da equipe, criar alguns
        if team_count == 0:
            print("[ℹ️] Criando membros da equipe de exemplo...")
            create_sample_team_members(cursor)

        # Criar dados faltantes
        if briefings_count == 0:
            print("[ℹ️] Criando briefings de exemplo...")
            create_sample_briefings(cursor)

        if timeline_count == 0:
            print("[ℹ️] Criando timeline de exemplo...")
            create_sample_timeline(cursor)

        if deliverables_count == 0:
            print("[ℹ️] Criando entregas de exemplo...")
...
def create_sample_events(cursor):
    ...
    # Se não houver clientes, criar alguns
    if not client_ids:
